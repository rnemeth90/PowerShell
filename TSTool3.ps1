...
# Source File: TSTool3.0.24.pff
#------------------------------------------------------------------------
#========================================================================
# Code Generated By: SAPIEN Technologies, Inc., PowerShell Studio 2012 v3.0.7
# Generated On: 10/02/2013 5:35 PM
# Generated By: admin
#========================================================================
Param(  
	[string]$BaseUser = $env:userName,
	[string]$RunningAs
)
...
function OnApplicationLoad {
	...
	set-variable -name TSToolVersion -value "3.0.24" -scope script -description "TSTool Version" 	
	...
	function global:Get-RegKey
	{
        ...
        if($BaseUser)
		{
			try
			{
				# Attempt to 'SET' this local key in 'base user' registry (user that ran script) contex.
				# Required if runas different admin account because the "RunAsAdmin" key needs to be read by base user.
				
				$Computer = $env:ComputerName

				$objUser = New-Object System.Security.Principal.NTAccount($BaseUser)
				$SID = $objUser.Translate([System.Security.Principal.SecurityIdentifier])
				$BaseUserSID = $SID.Value
	            
	            $regUsers= [Microsoft.Win32.RegistryKey]::OpenRemoteBaseKey('Users', $computer)    
			    $regKey = $regUsers.OpenSubKey("$BaseUserSID\\Software\\PowerShell\\TSTool",$true)    
			...
			{
				# ignore, base user and runas user are probably the same, i.e user is a local admin
			}
		}
	    ...
	    if (-not(Test-Path -Path $Reg))
	    {
	        ...
	        #Get-RegKey -Key $key -Value 0 -Set -BaseUser $BaseUser		
	    }
	...
	if ((get-host).name -eq "ConsoleHost" -or (get-host).name -eq "Windows PowerShell ISE Host")
	{
		...
		# Use RunAS if user preference set in registry
		if ([int](Get-RegKey -Key 'UseRunAs'))
		{
		    # Use-RunAs function passes the $RunningAs variable to script as an argument.
            # Check $RunningAs to prevent script going into endless re-launch as admin loop
	...
	if (-not($STA))
	{
		...
		#[System.Reflection.Assembly]::LoadWithPartialName("System.Windows.Forms") | Out-Null
		#[Windows.Forms.MessageBox]::Show("TSTool3 requires to be run in -STA mode`n`nSuggest you reinstall TSTool3 shortcut using the install.vbs`nthat was distributed with this version of the application.","TSTool $TSToolVersion")		
		#exit
	}
	
	
	#Check screen reesolution is at least 1024 wide    
    try
    {
        $ScreenWidthPass = $false
        $ScreenWidthProperty = $false
        $desktops = Get-WmiObject -Class Win32_DesktopMonitor -ComputerName $env:computername -ErrorAction 'stop'
        foreach ($desktop in $desktops) 
        {
            if ($desktop.ScreenWidth -gt 1024)
            {
                $ScreenWidthPass = $true
            }
            if ($desktop.ScreenWidth -gt 0)
            {
                $ScreenWidthProperty = $true
            }
        }
    }
    catch
    {
  	...
  	# display warning if screen resolution is returned but is 1024W or less
    if ($ScreenWidthProperty-and !($ScreenWidthPass))
    {
        [System.Reflection.Assembly]::LoadWithPartialName("System.Windows.Forms") | Out-Null
        [Windows.Forms.MessageBox]::Show("TSTool3 requires a screen resolution greater than 1024 wide","TSTool $TSToolVersion")
        # $formTSTool3.Close() 
    }
    ...
    {		
 	...
 	if ((get-host).name -eq "ConsoleHost" -or (get-host).name -eq "Windows PowerShell ISE Host")
	{
	    # Script has been run or re-run as admin (posibly using different credentials) therefore this ensures the runas preference is saved in current user contex too. 
	    if (Use-RunAs -check)
	   	{
			# $foo = [Microsoft.VisualBasic.Interaction]::InputBox($BaseUser, "TSTool $TSToolVersion", $BaseUser)
			Get-RegKey -Key UseRunAs -Value 1 -Set -BaseUser $BaseUser 
		}
		else
		{
			# Script is not running as admin - Set runas preference to 'off' (except if runas failed or was aborted for some reason)
			if (!($RunAsFail))
			{ 
				# $foo = [Microsoft.VisualBasic.Interaction]::InputBox($BaseUser, "TSTool $TSToolVersion", $BaseUser)
				Get-RegKey -Key UseRunAs -Value 0 -Set -BaseUser $BaseUser 
			}
	
			[System.Reflection.Assembly]::LoadWithPartialName("System.Windows.Forms") | Out-Null
            $Answer = [Windows.Forms.MessageBox]::Show("This script is designed to be run from a least privilaged account,`nAnd then runas administrator (Preferably a domain type admin)`n`nPlease set RunAsAdmin under TSTool3 options menu.", "TSTool $TSToolVersion","OK","Exclamation")
		}
	}
	
	
	# Create Default CSV file if one does not exist in script's parent directory
...
#----------------------------------------------
function Call-TSTool3_0_24_pff {
	...
	$labelAddRemovePrograms = New-Object 'System.Windows.Forms.Label'
	$labelInstalledMSI = New-Object 'System.Windows.Forms.Label'
	$InstalledMSI = New-Object 'System.Windows.Forms.Button'
	$labelLIVE = New-Object 'System.Windows.Forms.Label'
	$labelPackageStatus = New-Object 'System.Windows.Forms.Label'
	$labelAllApps = New-Object 'System.Windows.Forms.Label'
	$labelFiltered = New-Object 'System.Windows.Forms.Label'
	$labelSCCMLive = New-Object 'System.Windows.Forms.Label'
	$AddRemovePrograms = New-Object 'System.Windows.Forms.Button'
	$buttonSW1 = New-Object 'System.Windows.Forms.Button'
	$SCCM_2007_SW3 = New-Object 'System.Windows.Forms.Button'
	$SCCM_2007_SW2 = New-Object 'System.Windows.Forms.Button'
	...
	$runAsAdminToolStripMenuItem = New-Object 'System.Windows.Forms.ToolStripMenuItem'
...
y+twgxDxAwxAgjZAtAEA+Nmc14IfYYIAAAAASUVORK5CYII=')
	...
	# default AD Image
	$picturebox3Image = [System.Convert]::FromBase64String('R0lGODlhkACQAIIAALW1tdra2snJyePj46urq7+/v6Ojo/Dw8CH/C05FVFNDQVBFMi4wAwEBAAAh
+QQAAAAAACwAAAAAkACQAAAI/wAPCBxIsKDBgwgTKlzIsKHDhxAjSpxIsaLFixgzatzIsaPHjyBD
...
')
	#endregion
	    ...
	    {
			# $foo = [Microsoft.VisualBasic.Interaction]::InputBox($BaseUser, "TSTool $TSToolVersion", $BaseUser)
			Get-RegKey -Key Count -Value 20 -Set -BaseUser $BaseUser
	    }
		...
		# Writing the $Domain variable here directly to status strip crashes the editor \/ workaround
		...
		# Display runas option selection image
		if ([int](Get-RegKey -Key UseRunAs))
		{
			$runAsAdminToolStripMenuItem.Image = $tick	
		}
		else
		{
			$runAsAdminToolStripMenuItem.Image = $Cross		
		}
		...
		# Retrieve SCCM Server settings
		if (Test-Path "$ParentDirectory\GlobalSettings.csv")
		{
		    $SettingsCSV = Import-Csv "$ParentDirectory\GlobalSettings.csv"
		    set-variable -name SCCM07Server -value $($SettingsCSV.SCCM07Server) -scope script -description "SCCM 2007 SiteServer" 
		    set-variable -name SCCM07NS -value $($SettingsCSV.SCCM07NS) -scope script -description "SCCM 2007 NameSpace" 
		    set-variable -name SCCM12Server -value $($SettingsCSV.SCCM12Server) -scope script -description "SCCM 2012 SiteServer" 
		    set-variable -name SCCM12NS -value $($SettingsCSV.SCCM12NS) -scope script -description "SCCM 2012 NameSpace" 
		}
	...
	$buttonStartJob_Click={
		...
		$TextBox1Validated = $textbox1.Text.Trim()
		$TextBox1Validated = $TextBox1Validated.Replace("`t", "")
		# regex validation here would be good
		
		if (-not($TextBox1Validated -eq $null -or $TextBox1Validate -eq ""))
		{
			$Computer = $TextBox1Validated.toupper()
			$textbox1.Text = $Computer
			...
			set-variable -name computer -value $Computer -scope script -description "Current Connected Computer variable set in script scope."
		}
	...
	$StartJob0_Connectivity={
		...
		-JobScript {
			...
			function Global:Test-WMI
			{
			    ...
			    if (Test-Connection -ComputerName $Computer -Count 1 -Quiet)
			    {
			        ...
			        $JobName = Start-Job -ArgumentList $Computer -ScriptBlock { "Test-WMI"; Get-WmiObject Win32_Bios -ComputerName $args}  | select -ExpandProperty Name
		...
		-CompletedScript {
			...
			if ($Connectivity.Ping -and $Connectivity.WMIAccess)
			{
				$picturebox2.BackColor = 'LimeGreen'
				
				# note labelBarcode was subtley changed when button pressed  
				if ($labelBarcode.Text -eq $Connectivity.Computer -and $label1.text -ne "")
				{
			...
			$labelBarcode.Text = $Connectivity.Computer
	...
	$StartJob1={
		...
		# User Specific Queries + uptime (non static type properties)
		...
		-JobScript {
	        ...
	        try
	        {
				...
				$UserID = Get-WmiObject Win32_Process -ComputerName $Computer -filter "Name = 'explorer.exe'" | foreach -process {$_.GetOwner().User} | Select-Object -First 1
		...
		-CompletedScript {
			...
			else
			{
	            ...
	            $HistoryObj = $HistoryObj | % { if( $_.Computer -ne $computer) { $_ } } 
	...
	$buttonProfileSvr_Click={
		...
		{
			Get-RegKey -Key profileServer -Value "scfs11" -Set -BaseUser $BaseUser
	...
	$InstalledMSI_Click={
		if ($picturebox2.BackColor.Name -eq 'LimeGreen' -and $buttonStartJob.Enabled -eq $true)
	    {
		...
		{		
			...
			-JobScript {
				...
				function Get-AddRemovePrograms($Computer)
				{
	                ...
	                $RemoteRegistry = gwmi -computername $computer -class win32_service | Where-Object { $_.Name -eq “RemoteRegistry” }
					if ($RemoteRegistry.State -eq "Stopped" -and $computer -ne $env:computername) 
					{
						$RemoteRegistry.StartService()
						"Attempting to start $computer remote registry service - Wait ~15 secs and try again"
					}
					else
					{				
						$array = @()
					    ...
					    $array | Where-Object { $_.Name } | select ComputerName, Name, Version, InstallDate | sort name -Unique
					} # end if
				}
...
"@
	...
	$buttonC_Click={
		...
		{
			explorer.exe "\\$($labelBarcode.text)\c$"	
		}
	}
	...
	$buttonWakeUp_Click={
	    ...
	    if ($picturebox2.BackColor.Name -eq 'Red')
	    {
	        $computer = $labelBarcode.text
	        
	        $ButtonJob = "Send-WOL -computer $computer -NameSpace $SCCM12NS -SiteServer $SCCM12Server -max 1"
	...
	$SCCM_2007_SW2_Click={
	    ...
	    if ($Computer -ne "")
	    {
			if ($SCCM07Server -ne $null)
			{
				Update-Display "SCCM 2007 HTML Report - All advertisements for $computer" -color Yellow
				
				$url="http://$SCCM07Server/SMSReporting_SC1/Report.asp?ReportID=126&ComputerName=$computer"
				...
				{
					Update-Display "Cant open html page" -color 'red'
				}
			}            
	    } 
	}
	...
	$SCCM_2007_SW3_Click={
	    ...
	    if ($Computer -ne "")
	    {
			...
			$SCCMServer = Get-SCCMResourceID -Computer $Computer -SiteServer1 $SCCM12Server -NameSpace1 $SCCM12NS -SiteServer2 $SCCM07Server -NameSpace2 $SCCM07NS
	        ...
	        {
	            Update-Display "Computer not found in SCCM" -color "Red"
	        }
	    }
	}
	...
	$buttonDisconnect_Click={
		...
		Update-Display "Cancelling remote job threads" -color "yellow"	
	...
	$runAsAdminToolStripMenuItem_Click={
		# Toggle the RunAs option
		if ([int](Get-RegKey -Key UseRunAs))
		{
			$runAsAdminToolStripMenuItem.Image = $Cross
		...
		else
		{
			...
			# if currently running as non privilaged user - offer restart. 
			if (!(Use-RunAs -check))
			{
				$Answer = [Windows.Forms.MessageBox]::Show("Restart TSTool as Administrator?", "TSTool $TSToolVersion","YesNo","Question")
				if( $Answer -eq "Yes" )
				{
					$arg = "-executionpolicy bypass -windowstyle hidden -sta -file `"$($MyInvocation.ScriptName)`""
			        Start-Process "$psHome\powershell.exe" -Verb Runas -ArgumentList $arg -ErrorAction 'stop' 
					$formTSTool3.Close()	
				}
			}		
	...
	$addQLDPhoneFilterToolStripMenuItem_Click={
		...
		{
			$addQLDPhoneFilterToolStripMenuItem.Image = $Cross	
			Get-RegKey -Key UseQLDFilter -Value 0 -Set -BaseUser $BaseUser 
		}
		else
		{
			$addQLDPhoneFilterToolStripMenuItem.Image = $tick
			Get-RegKey -Key UseQLDFilter -Value 1 -Set -BaseUser $BaseUser 
		}
	...
	$setHistoryLimitToolStripMenuItem_Click={
	    ...
	    try
		{
		    ...
		    {
		 		Get-RegKey -Key Count -Value $HistoryCount -Set -BaseUser $BaseUser
		    }
		}
		catch
		{}
	...
	$setProfileServerToolStripMenuItem_Click={
		...
		Get-RegKey -Key profileServer -Value $profileServer -Set -BaseUser $BaseUser
	}
	...
	$buttonSCCMU2M_Click={
		...
		Function Get-SCCMComputer
		{
			...
			{
				Update-Display "SCCM 2012 Server not set in GlobalSettings.csv file" -color red		
			}
		}
		
		Update-Display (Get-SCCMComputer -NameSpace $SCCM12NS -SCCMServer $SCCM12Server | sort creationtime | ft user, computer, creationTime -AutoSize)
	...
	$Form_Cleanup_FormClosed=
	{
		...
		try
		{
			...
			$buttonSW1.remove_Click($buttonSW1_Click)
			$SCCM_2007_SW3.remove_Click($SCCM_2007_SW3_Click)
			$SCCM_2007_SW3.remove_MouseHover($ShowHelp)
			$SCCM_2007_SW2.remove_Click($SCCM_2007_SW2_Click)
			$SCCM_2007_SW2.remove_MouseHover($ShowHelp)
			...
			$runAsAdminToolStripMenuItem.remove_Click($runAsAdminToolStripMenuItem_Click)
...
"
	...
	$tabpage2.Controls.Add($labelAddRemovePrograms)
	$tabpage2.Controls.Add($labelInstalledMSI)
	$tabpage2.Controls.Add($InstalledMSI)
	$tabpage2.Controls.Add($labelLIVE)
	$tabpage2.Controls.Add($labelPackageStatus)
	$tabpage2.Controls.Add($labelAllApps)
	$tabpage2.Controls.Add($labelFiltered)
	$tabpage2.Controls.Add($labelSCCMLive)
	$tabpage2.Controls.Add($AddRemovePrograms)
	$tabpage2.Controls.Add($buttonSW1)
	$tabpage2.Controls.Add($SCCM_2007_SW3)
	$tabpage2.Controls.Add($SCCM_2007_SW2)
	...
	# buttonWin8Apps
	#
	$buttonWin8Apps.Location = '579, 102'
	...
	#
	# labelAddRemovePrograms
	#
	$labelAddRemovePrograms.Location = '790, 102'
	$labelAddRemovePrograms.Name = "labelAddRemovePrograms"
	$labelAddRemovePrograms.Size = '121, 23'
	$labelAddRemovePrograms.TabIndex = 29
	$labelAddRemovePrograms.Text = "Add Remove Programs"
	$labelAddRemovePrograms.TextAlign = 'MiddleLeft'
	#
	# labelInstalledMSI
	#
	$labelInstalledMSI.Location = '790, 72'
	$labelInstalledMSI.Name = "labelInstalledMSI"
	$labelInstalledMSI.Size = '75, 23'
	$labelInstalledMSI.TabIndex = 28
	$labelInstalledMSI.Text = "Installed MSI"
	$labelInstalledMSI.TextAlign = 'MiddleLeft'
	#
	# InstalledMSI
	#
	...
	# labelPackageStatus
	#
	$labelPackageStatus.Location = '340, 72'
	$labelPackageStatus.Name = "labelPackageStatus"
	$labelPackageStatus.Size = '93, 23'
	$labelPackageStatus.TabIndex = 21
	$labelPackageStatus.Text = "Package Status"
	$labelPackageStatus.TextAlign = 'MiddleRight'
	#
	# labelAllApps
	#
	$labelAllApps.Location = '362, 102'
	$labelAllApps.Name = "labelAllApps"
	$labelAllApps.Size = '68, 23'
	$labelAllApps.TabIndex = 20
	$labelAllApps.Text = "All Apps"
	$labelAllApps.TextAlign = 'MiddleRight'
	#
	# labelFiltered
	#
	$labelFiltered.Location = '790, 42'
	$labelFiltered.Name = "labelFiltered"
	$labelFiltered.Size = '51, 23'
	$labelFiltered.TabIndex = 19
	$labelFiltered.Text = "Filtered"
	$labelFiltered.TextAlign = 'MiddleLeft'
	#
	# labelSCCMLive
	#
	...
	$AddRemovePrograms.Text = "Registry"
	$AddRemovePrograms.UseVisualStyleBackColor = $True
	$AddRemovePrograms.add_Click($AddRemovePrograms_Click)
	#
	# buttonSW1
	#
	$buttonSW1.Location = '709, 42'
	$buttonSW1.Name = "buttonSW1"
	$buttonSW1.Size = '75, 23'
	$buttonSW1.TabIndex = 16
	$buttonSW1.UseVisualStyleBackColor = $True
	$buttonSW1.add_Click($buttonSW1_Click)
	#
	# SCCM_2007_SW3
	#
	...
	# SCCM_2007_SW2
	#
	$SCCM_2007_SW2.Location = '439, 72'
	$SCCM_2007_SW2.Name = "SCCM_2007_SW2"
	$SCCM_2007_SW2.Size = '75, 23'
	$SCCM_2007_SW2.TabIndex = 14
	$SCCM_2007_SW2.Text = "SW2"
	$SCCM_2007_SW2.UseVisualStyleBackColor = $True
	$SCCM_2007_SW2.add_Click($SCCM_2007_SW2_Click)
	$SCCM_2007_SW2.add_MouseHover($ShowHelp)
	#
	# tabpage4
	#
...
Mobile"
	...
	#
	[void]$fileToolStripMenuItem.DropDownItems.Add($runAsAdminToolStripMenuItem)
...

...

...
AT8E/wH8AT8B/AE/Cw=='))
	...
	#
	#region Binary Data
	$runAsAdminToolStripMenuItem.Image = [System.Convert]::FromBase64String('')
	...
	$runAsAdminToolStripMenuItem.Size = '173, 22'
	$runAsAdminToolStripMenuItem.Text = "RunAsAdmin"
	$runAsAdminToolStripMenuItem.add_Click($runAsAdminToolStripMenuItem_Click)
	#
...
{
	#Call the form
	Call-TSTool3_0_24_pff | Out-Null
	#Perform cleanup
	OnApplicationExit
}
